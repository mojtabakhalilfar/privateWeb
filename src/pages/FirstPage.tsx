...
// style={{ backgroundImage: "url(/assets/image/prof.jpg)" }}

    return (
        <Container id='home' >
            <div  className="h-[696px] w-full mt-14 sm:mt-18 flex flex-col-reverse sm:flex-row-reverse justify-start items-start bg-black">
                        ...
                        <div className='flex flex-col justify-around sm:flex-row sm:items-center sm:justify-start px-4 h-full sm:h-3/12 sm:w-4/5 py-12'>
                            {
                                socialNetwork.map((items, index) => (
                                    <a key={index} className='w-5 sm:w-14 aspect-square mx-1 bg-[#C4C4C4] flex items-center justify-center shadow-[0_4px_4px_rgba(0,0,0,0.25)]' href={items.href}><img src={items.image} alt={items.alt} /></a>
                                ))
                            }
                        </div>
                    </div>
